...
def conv_layer(in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, device=None):
...
class EfficientSelfAttention(nn.Module):
    def __init__(
        ...
        *,
        dim,
        heads,
        reduction_ratio, device=None
    ):
        super().__init__()
        self.scale = (dim // heads) ** -0.5
        self.heads = heads

        self.to_q = nn.Conv2d(dim, dim, 1, bias = False, device=device)
        self.to_kv = nn.Conv2d(dim, dim * 2, reduction_ratio, stride = reduction_ratio, bias = False, device=device)
        self.to_out = nn.Conv2d(dim, dim, 1, bias = False, device=device)
...
class CCALayer(nn.Module):
    def __init__(self, channel, reduction=4, device=None):
        super(CCALayer, self).__init__()

        self.esa = EfficientSelfAttention(dim=64,heads=2,reduction_ratio=reduction, device=device)

        hidden_dim = channel * 8
        self.conv3 = nn.Sequential(
            nn.Conv2d(channel, hidden_dim, 1, device=device),
            nn.GELU(),
            nn.Conv2d(hidden_dim, channel, 1, device=device),
        )
...
class SIMDB(nn.Module):#OLD
    def __init__(self, in_channels, distillation_rate=0.25, device=None):
        ...
        self.c1=nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, device=device)
        self.c2=nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, device=device)
        self.c3=nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1, device=device)
        self.c4=nn.Conv2d(in_channels, self.distilled_channels, kernel_size=3, stride=1, padding=1, device=device)
        self.act = activation('lrelu', neg_slope=0.05)
        # self.c5 = conv_layer(in_channels, in_channels, 1)
        self.c6 = conv_layer(in_channels, self.distilled_channels, 1, device=device)
...
class FPN(Module):
    def __init__(
        self, 
        ch_in,  # 输入张量x和y的通道数 The number of channels of the input tensors x and y
        ch_out: int, device=None, # 输出张量的通道数 The number of channels of the output tensor
    ):
        super().__init__()
        if ch_in is None:
        ...
        self.convs1_1=nn.Sequential(
            ...
            nn.Conv2d(self.ch_in[0], self.ch3, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch3, device=device),
            nn.ReLU(inplace=True)
        )
        self.convs1_2=nn.Sequential(
            nn.Conv2d(self.ch_in[1], self.ch2, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch2, device=device),
            nn.ReLU(inplace=True),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)  # 添加上采样层
        )
        self.convs1_3=nn.Sequential(
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(self.ch_in[2], self.ch2, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch2, device=device),
            nn.ReLU(inplace=True),
        ...
        self.convs1_4=nn.Sequential(
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(self.ch_in[3], self.ch1, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch1, device=device),
            nn.ReLU(inplace=True),
        ...
        self.convs2_1=nn.Sequential(
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(self.ch_in[0], self.ch2, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch2, device=device),
            nn.ReLU(inplace=True)
        )
        self.convs2_2=nn.Sequential(
            nn.Conv2d(self.ch_in[1], self.ch3, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch3, device=device),
            nn.ReLU(inplace=True),
            # nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)  # 添加上采样层
        )
        ...
        self.convs3_1=nn.Sequential(
            ...
            nn.Conv2d(self.ch_in[0], self.ch1, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch1, device=device),
            nn.ReLU(inplace=True)
        )
        ...
        self.convs3_3=nn.Sequential(
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(self.ch_in[2], self.ch3, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch3, device=device),
            nn.ReLU(inplace=True),
            # nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)  # 添加上采样层
        )
        self.convs3_4=nn.Sequential(
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(self.ch_in[3], self.ch2, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch2, device=device),
            nn.ReLU(inplace=True),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)  # 添加上采样层
        )
        ...
        self.convs4_4=nn.Sequential(
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(self.ch_in[3], self.ch3, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.ch3, device=device),
            nn.ReLU(inplace=True),
            # nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)  # 添加上采样层
        )
        ...
        self.conv_aggregation_s1= FeatureFusionModule(self.chfusein, self.ch_in[0], self.ch_out, device=device)
        self.conv_aggregation_s2= FeatureFusionModule(self.chfusein, self.ch_in[1], self.ch_out, device=device)
        self.conv_aggregation_s3= FeatureFusionModule(self.chfusein, self.ch_in[2], self.ch_out, device=device)
        self.conv_aggregation_s4= FeatureFusionModule(self.chfusein, self.ch_in[3], self.ch_out, device=device)
...
class FeatureFusionModule(nn.Module):
    def __init__(self, fuse_d, id_d, out_d, device=None):
        super(FeatureFusionModule, self).__init__()
        self.fuse_d = fuse_d
        self.id_d = id_d
        self.out_d = out_d
        self.conv_fuse = nn.Sequential(
            nn.Conv2d(self.fuse_d, self.out_d, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.out_d, device=device),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.out_d, self.out_d, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.out_d, device=device),
        )
        self.conv_identity = nn.Conv2d(self.id_d, self.out_d, kernel_size=1, device=device)
        self.relu = nn.ReLU(inplace=True)
        # self.esa = EfficientSelfAttention(dim=64,heads=2,reduction_ratio=4)
...
class FFm(nn.Module):
    def __init__(self, in_d=64, out_d=64, device=None):
        super(FFm, self).__init__()
        self.in_d = in_d
        self.out_d = out_d
        self.relu = nn.ReLU(inplace=True)
        self.sigmoid=nn.Sigmoid()
        self.conv1=nn.Conv2d(16, self.in_d, kernel_size=1, device=device)


        self.msff = SIMDB(64, device=device)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
...
class Decoder(nn.Module):
    def __init__(self, mid_d=320, device=None):
        ...
        self.conv_p4 = nn.Sequential(
            nn.Conv2d(self.mid_d, self.mid_d, kernel_size=3, stride=1, padding=1, device=device),
            nn.BatchNorm2d(self.mid_d, device=device),
            nn.ReLU(inplace=True)
        )
        ...
        self.cls = nn.Conv2d(self.mid_d, 1, kernel_size=1, device=device)
        

        self.cca = CCALayer(64, device=device)
...
class BaseNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=1, device=None):
        ...
        self.ffm=FFm(self.mid_d, self.en_d * 2, device=device)
        self.decoder = Decoder(self.en_d * 2, device=device)
        # self.msff = SIMDB(64)
